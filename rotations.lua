...
local function play_note(note_val, ch)
    if note_val ~= nil then
        ...
        midi_note_on(60 + scale_note, MIDI_VELOCITY, ch)
    end
end
...
local function stop_note(note_val, ch)
    if note_val ~= nil then
        ...
        midi_note_off(48 + scale_note, MIDI_VELOCITY, ch)
    end
end
...
function arc(n, d)
    if key1_held then
        if d > 0 then
            -- add a new random note at a random position
            local ranges = {
                { 1,  11 },
                { 12, 23 },
                { 24, 35 },
            ...
            -- store the position as a marker to allow undo
